 ...
 * (use the find/getMore commands), "commandsNoDocumentSequences" (use the find/getMore commands
 * without document sequencs) "legacy" (use legacy OP_QUERY/OP_GET_MORE wire protocol reads), or 
 * "compatibility" (auto-detect mode based on wire version).
 */
Mongo.prototype.forceReadMode = function(mode) {
    if (mode !== "commands" && mode !== "commandsNoDocumentSequences" && mode !== "compatibility" && mode !== "legacy") {
...
Mongo.prototype.readMode = function() {
    ...
    if (this.hasOwnProperty("_readMode") && this._readMode !== "compatibility") {
        ...
        try {
            ...
            var hasReadCommandsNoDocumentSequences = (this.getMinWireVersion() <= 4 && 4 <= this.getMaxWireVersion());
            if (hasReadCommands) {
